<div class="container">
	<section class="pull-right">
		<div class="well-sm">
			<?= $this->html->link($config['url']); ?>
		</div>
	</section>

	<ul class="breadcrumb">
		<li><?=$this->html->link('Home', '/');?></li>
		<li><?=$this->html->link('Docs', '/docs');?></li>
		<li><?=$this->html->link($config['name'], sprintf('/docs/%s', $library));?></li>
		<?php if (stristr($slug, '/')): $parts = explode('/', $slug); ?>
			<?php foreach ($parts as $part): ?>
				<li class="active"><?= \lithium\util\Inflector::humanize(pathinfo($part, PATHINFO_FILENAME)); ?></li>
			<?php endforeach; ?>
		<?php else: ?>
			<li class="active"><?=$this->title(pathinfo($file, PATHINFO_FILENAME)); ?></li>
		<?php endif; ?>
	</ul>
</div>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-3 col-sm-3 border-right">
				<?= $this->_render('element', 'docs/single'); ?>
			</div>
...
<section class="row" id="documentation">
 	<div class="col-md-3">

		<div class="list-group">
			<?php foreach ($files as $file): $file = trim($file, '/');
			$class = ($file == $filename) ? 'list-group-item active' : 'list-group-item'; ?>
				<a href="<?= $this->url(sprintf('/docs/%s/%s', $library, $file)); ?>" class="<?= $class ?>">
					<?= trim(str_replace('/', ' / ', $file), '.md'); ?>
				</a>
			<?php endforeach; ?>
		</div>
		&nbsp;
...
<?php echo $this->html->style('/li3_bootstrap/css/rainbow'); ?>
<script type="text/javascript" charset="utf-8">
head.ready('jquery', function() {
	head.js({toc: "<?php echo $this->url('/js/toc.min.js'); ?>"});
});
head.ready('toc', function() {
	docs = {}
	docs.slugify = function(v) {
		if (!v) return '';
	...
	docs.init = function() {
		...
		$('#toc').toc({
			...
			anchorName: function(i, heading, prefix) { //custom function for anchor name
				return docs.slugify(heading.innerHTML);
			},
